...
class UserAdmin(UrlActionMixin, GrainyUserAdmin):
    ...
    def get_queryset(self, request):
        ...
        if is_impersonating(request):
            self.message_user(request, f"Currently impersonating {is_impersonating(request)}", messages.INFO)
    ...
    def impersonate(self, obj):
        ...
        with current_request() as request:
            ...
            if user == obj:
                url = reverse("admin:auth_user_actions", args=(obj.id, "stop_impersonation"))
    ...
    def start_impersonation(self, request, queryset):
        if queryset.count() > 1:
            self.message_user(request, "Please only select one user to impersonate", messages.ERROR)
