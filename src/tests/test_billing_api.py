...
def test_get_payment_methods(billing_objects, data_billing_api_billcon):
    ...
    response = billing_objects.api_client.get(
        reverse("billing_api:org-payment-methods", args=[billing_objects.org.slug]) + f"?billcon={billing_objects.billing_contact.id}"
    )
    print(response.content)

    assert strip_api_fields(response.json()) == strip_api_fields(data_billing_api_billcon.expected)
...
def test_delete_active_billing_contact(billing_objects):
    ...
    assert error_message in json.loads(response.content)["errors"]["non_field_errors"][0]
...
def test_get_services(billing_objects, data_billing_api_subscriptions):
    ...
    assert strip_api_fields(response.json()) == strip_api_fields(data_billing_api_subscriptions.expected)
...
def test_get_products(billing_objects, data_billing_api_prod):
    ...
    assert response.status_code == 200
    assert strip_api_fields(response.json()) == strip_api_fields(data_billing_api_prod.expected)
